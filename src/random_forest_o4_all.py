...
# === CONFIGURATION ===
CSV_PATHS = [
    "dataset/1st_capture/cpe_a-cpe_b-mobile.csv",
    "dataset/2nd_capture/cpe_a-cpe_b-mobile.csv"
]
LOOKBACK = 20
PRED_WINDOW = 5
...
df_all = pd.concat([load_and_preprocess(p) for p in CSV_PATHS], ignore_index=True)
...
def generate_dataset_with_emphasized_tail(df, lookback, pred_window):
    ...
    for i in range(lookback, len(df) - pred_window):
        ...
        tail_repeated = tail_features * 3  # 可调整重复次数
