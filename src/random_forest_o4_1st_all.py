...
# === CONFIGURATION ===
CSV_FOLDER = "dataset/1st_capture/*.csv"
LOOKBACK = 10
PRED_WINDOW = 1
...
csv_files = glob(CSV_FOLDER)
print(f"Loading {len(csv_files)} files from {CSV_FOLDER}")
df_all = pd.concat([load_and_preprocess(f) for f in csv_files], ignore_index=True)
...
def generate_dataset_with_emphasized_tail(df, lookback, pred_window):
    ...
    for i in range(lookback, len(df) - pred_window):
        ...
        tail_features = [rolling_std_last_5, rolling_mean_diff_last_3]
        tail_repeated = tail_features * 3
...
# === Feature Generation ===
X, y, indices = generate_dataset_with_emphasized_tail(df_all, lookback=LOOKBACK, pred_window=PRED_WINDOW)
...
feature_names = (
    ...
    ["mean", "std", "min", "max", "range", "median",
     "slope", "last_delta", "coef_var"] +
    ["rolling_std_last_5", "rolling_mean_diff_last_3"] * 3
)
